        ...
        try:
            ...
            for item in items:
                ...
                for i in range(1, len(filter_rules)):
                    rule = filter_rules[i]; op = rule.get("operator", "AND"); target = rule.get("target", "name")
                    target_val = item_name if target == "name" else (item_path if target == "path" else item_content)
...
def test_filter_logic_basic():
    ...
    # 5. Regex
    rules5 = [json.dumps([
        {"target": "content", "mode": "regex", "not": False, "value": "h.llo"}
    ])]
    contents, jsons = filter_node.process(json_list, rules5)
    assert len(jsons) == 1
    assert json.loads(jsons[0])["name"] == "test1.txt"
