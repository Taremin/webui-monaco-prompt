...
def test_settings_sync_between_editors(page: Page, comfyui_server, wait_for_comfyui, wmp_helpers):
    ...
    font_sizes = page.evaluate("""() => {
        ...
        return nodes.map((n, i) => {
            if (n.multitext_widget && n.multitext_widget.editorInstance) {
                return n.multitext_widget.editorInstance.getSettings().fontSize;
            }
            return null;
        });
    }""")
    ...
    wmp_helpers.set_comfy_setting(page, 'WebuiMonacoPrompt.FontSize', 18)
    
    page.wait_for_function("""() => {
        const findApp = () => window.app || (window.comfyAPI && window.comfyAPI.app) || window.ComfyApp;
        const app = findApp();
        const nodes = app.graph._nodes.filter(n => n.type && n.type.includes('MultiText'));
        return nodes.every(n => n.multitext_widget && n.multitext_widget.editorInstance && n.multitext_widget.editorInstance.getSettings().fontSize === 18);
    }""")
    ...
    new_font_sizes = page.evaluate("""() => {
        ...
        const app = findApp();
        const nodes = app.graph._nodes.filter(n => n.type && n.type.includes('MultiText'));
        return nodes.map(n => n.multitext_widget.editorInstance.getSettings().fontSize);
    }""")
    assert new_font_sizes[0] == 18 and new_font_sizes[1] == 18, "Both editors should reflect the new font size."

def test_dynamic_csv_toggle_sync(page: Page, comfyui_server, wait_for_comfyui, wmp_helpers):
    page.set_viewport_size({"width": 1920, "height": 1080})
    wmp_helpers.load_comfyui(page, comfyui_server, wait_for_comfyui)
